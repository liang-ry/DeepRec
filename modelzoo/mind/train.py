...
HIST_COLUMN = ['tag_category_list', 'tag_brand_list'] 
INPUT_FEATURES = [
...
class MIND():
    def __init__(self,
        ...
        self.concat_dnn_hidden_units = concat_dnn_hidden_units
    ...
    def _dnn_tower(self, net, name, hiddens):
    ...
    def _capsule_layer(self, seq_feas, seq_lens):
        ...
        if self.bf16:
            routing_logits = tf.cast(routing_logits, dtype=tf.bfloat16)
        ...
        if self.bf16:
            bilinear_matrix = tf.get_variable(
                dtype=tf.bfloat16, shape=[low_fea_dim, 64], name='capsule/S')
        else:
            bilinear_matrix = tf.get_variable(
                dtype=tf.float32, shape=[low_fea_dim, 64], name='capsule/S')
        ...
        if self.bf16:
            mask_cap = tf.cast(mask_cap, tf.bfloat16)
            max_cap_thresh = (tf.cast(mask_cap[:, None, :], tf.bfloat16) * 2 - 1) * 1e32
        else:
        ...
        for iter_id in range(3):
            ...
            high_capsules = tf.einsum(
                ...
                if iter_id + 1 < 3 else seq_feas_high, routing_logits)
        ...
        if self.bf16:
            high_capsules = high_capsules * tf.cast(high_capsule_mask[:, :, None], dtype=tf.bfloat16)
        else:
    ...
    def _create_model(self):
        ...
        hist_seq_feas = [x[0] for x in seq_features]
        hist_seq_len = seq_features[0][1]
        ...
        if self.bf16:
            ...
            hist_seq_feas = tf.cast(hist_seq_feas, dtype=tf.bfloat16)
            hist_seq_len = tf.cast(hist_seq_len, dtype=tf.bfloat16)
        high_capsules, num_high_capsules = self._capsule_layer(hist_seq_feas,
                                                     hist_seq_len)
        user_features = tf.layers.batch_normalization(
                ...
                trainable=True,
                name='user_fea_bn')
        user_features = self._dnn_tower(user_features,name='user',hiddens=self._dnn_hidden_units)
        ...
        num_concat_dnn_layer = len(self.concat_dnn_hidden_units)
        last_hidden = self.concat_dnn_hidden_units.pop()
        user_interests = self._dnn_tower(user_interests,name='concat',hiddens=self.concat_dnn_hidden_units)
        user_interests = tf.layers.dense(
            inputs=user_interests,
            units=last_hidden,
            name='concat_dnn/dnn_%d' % (num_concat_dnn_layer - 1))

        item_tower_emb = self._dnn_tower(item_emb,name='item',hiddens=self._dnn_hidden_units)
        item_tower_emb = tf.layers.dense(
            inputs=item_tower_emb,
            units=self._dnn_last_hidden_units,
            name='item_dnn/dnn_%d' % (4 - 1))

        item_tower_emb = tf.math.l2_normalize(item_tower_emb, axis=1)
        user_interests = tf.math.l2_normalize(user_interests, axis=1)
        ...
        simi = simi
        simi_mask = tf.sequence_mask(num_high_capsules, 5)
        if self.bf16:
            user_interests = user_interests * tf.cast(simi_mask[:, :, None],dtype=tf.bfloat16)
            max_thresh = (tf.cast(simi_mask, tf.bfloat16) * 2 - 1) * 1e32
        else:
        ...
        user_item_sim = tf.reduce_sum(tf.multiply(user_tower_emb, item_tower_emb), axis=1, keep_dims=True)
        if self.bf16:
            ...
            sim_b = tf.get_variable('sim_b',
                                ...
                                shape=(1),
                                initializer=tf.zeros_initializer())
        self._logits = user_item_sim * tf.abs(sim_w) + sim_b
...
class _LazyBuilder(object):
    ...
    def _get_raw_feature_as_tensor(self, key):
        ...
        feature_tensor = tf.python.sparse_tensor.convert_to_tensor_or_sparse_tensor(
            raw_feature)

        def expand_dims(input_tensor):
        # Input_tensor must have rank 1.
            if isinstance(input_tensor, tf.python.sparse_tensor.SparseTensor):
...
def build_feature_columns():
    ...
    for column_name in INPUT_FEATURES:
        ...
        elif column_name in HIST_COLUMN:
            hist_column.append(embedding_column)

    return user_column, item_column, hist_column
...
def main(tf_config=None, server=None):
    ...
    user_column, item_column, hist_column = build_feature_columns()
    ...
    # create model
    model = MIND(user_column=user_column,
