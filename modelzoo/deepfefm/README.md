...
## Model Structure
[DeepFEFM](https://arxiv.org/pdf/2009.09931v2.pdf) is a CRT recommender model proposed in 2021 which M learns symmetric matrix embeddings for each field pair along with the usual single vector embeddings for each feature. Besides, DeepFEFM combines the FEFM interaction vectors learned by the FEFM component with a DNN and is thus able to learn higher order interactions.
The model's output is the probability of a click calculated by the output of FEFM and DNN model.
```
output:
...
2.  Training.  
    ...
    - Choice Settings:
      - `--use_deep`: Use True represents `DeepFEFM`; False represents `FEFM`.
