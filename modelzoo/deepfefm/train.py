...
class DeepFEFM():
    def __init__(self,
        ...
        self._fefm_column = fefm_column
        if not wide_column or not fefm_column or not deep_column:
            raise ValueError(
    ...
    def _fefm(self, inputs, fields=None, layer_name='field_embeddings'):
        num_fields = fields
        embedding_size = inputs.shape[2]   # inputs: [batch_size, fields, embedding_size]
        "Define the features' embedding"
        field_embeddings = {}
        for fi, fj in itertools.combinations(range(num_fields), 2):
            field_pair_id = str(fi) + "-" + str(fj)
            if self.bf16:
                field_embeddings[field_pair_id] = tf.get_variable(layer_name + field_pair_id, 
                        (embedding_size, embedding_size),
                        initializer=tf.initializers.glorot_uniform(),
                        regularizer=tf.keras.regularizers.l1_l2(1e-3),
                        trainable=True,dtype=tf.bfloat16)
            else:
                field_embeddings[field_pair_id] = tf.get_variable(layer_name + field_pair_id, 
                        (embedding_size, embedding_size),
                        initializer=tf.initializers.glorot_uniform(),
    ...
    def _create_model(self):
        ...
        with tf.variable_scope('input_layer',
            ...
            if self._adaptive_emb and not self.tf:
                ...
                self._feature, self._wide_column, cols_to_output_tensors=fefm_cols, adaptive_mask_tensors=adaptive_mask_tensors)
            else:
                dnn_input = tf.feature_column.input_layer(self._feature,
                                                          self._deep_column)
                wide_input = tf.feature_column.input_layer(
                self._feature, self._wide_column, cols_to_output_tensors=fefm_cols)

            fefm_input = tf.stack([fefm_cols[cols] for cols in self._fefm_column], 1)

        if self.bf16:
            wide_input = tf.cast(wide_input, dtype=tf.bfloat16)
            fefm_input = tf.cast(fefm_input, dtype=tf.bfloat16)
        ...
        # fefm layer
        with tf.variable_scope('fefm', reuse=tf.AUTO_REUSE) as fefm:
            fefm_output = self._fefm(fefm_input, fields=len(self._fefm_column), layer_name='fefm_layer')

        # DNN part
        dnn_scope = tf.variable_scope('dnn')
        with dnn_scope.keep_weights(dtype=tf.float32) if self.bf16 \
                else dnn_scope:
            dnn_output = self._dnn(tf.concat((dnn_input,fefm_output),1), self._dnn_hidden_units,
                                   'dnn_layer')

        # Final dnn layer
        if self.use_deep: # DEEP-FEFM
            all_input = tf.concat([dnn_output, linear_output, fefm_output], 1)
        else:             # FEFM
            all_input = tf.concat([linear_output, fefm_output], 1)
...
def main(tf_config=None, server=None):
    ...
    model_dir = os.path.join(args.output_dir,
                             'model_DeepFEFM_' + str(int(time.time())))
    ...
    wide_column, fefm_column, deep_column = build_feature_columns()
    ...
    # create model
    model = DeepFEFM(wide_column=wide_column,
                   fefm_column=fefm_column,
...
def get_arg_parser():
    ...
    parser.add_argument('--workqueue',
                        ...
                        type=boolean_string,
                        default=False),
    parser.add_argument('--use_deep',
                        help='Whether to enable deep module(Setting True is Deep-FEFM, and the False is FEFM).',
                        type=boolean_string,
                        default=True)
    return parser
