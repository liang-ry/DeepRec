...
class xDeepFM():
    def __init__(self,
        ...
        self._cin_column = cin_column
        if not wide_column or not cin_column or not deep_column:
            raise ValueError(
        ...
        self._cin_hidden_units = cin_hidden_units
    ...
    def _cin(self,inputs,cin_size=None,fields=None):
        # inputs: [None, n, k]
        field_num = [fields] + cin_size # Number of matrices per layer (including layer 0)
        if self.bf16:
            cin_W = [tf.get_variable('w'+str(i), (1, field_num[0]*field_num[i], field_num[i+1]),
        ...
        k = inputs.shape[-1]
        res_list = [inputs]
        X0 = tf.split(inputs, k, axis=-1)           # The final dimension is cut into k parts，list: k * [None, field_num[0], 1]
        for i, size in enumerate(field_num[1:]):
    ...
    def _create_model(self):
        ...
        with tf.variable_scope('input_layer',
            ...
            if self._adaptive_emb and not self.tf:
                ...
                self._feature, self._wide_column, cols_to_output_tensors=cin_cols, adaptive_mask_tensors=adaptive_mask_tensors)
            else:
                dnn_input = tf.feature_column.input_layer(self._feature,
                                                          self._deep_column)
                wide_input = tf.feature_column.input_layer(
                self._feature, self._wide_column, cols_to_output_tensors=cin_cols)

            cin_input = tf.stack([cin_cols[cols] for cols in self._cin_column], 1)

        if self.bf16:
            wide_input = tf.cast(wide_input, dtype=tf.bfloat16)
            cin_input = tf.cast(cin_input, dtype=tf.bfloat16)
        ...
        # cin layer
        with tf.variable_scope('cin', reuse=tf.AUTO_REUSE) as cin:
            cin_output = self._cin(cin_input,self._cin_hidden_units,len(self._cin_column))

        # Final dnn layer
        all_input = tf.concat([dnn_output, linear_output, cin_output], 1)
...
def main(tf_config=None, server=None):
    ...
    model_dir = os.path.join(args.output_dir,
                             'model_XdeepFM_' + str(int(time.time())))
    ...
    wide_column, cin_column, deep_column = build_feature_columns()
    ...
    # create model
    model = xDeepFM(wide_column=wide_column,
                   cin_column=cin_column,
