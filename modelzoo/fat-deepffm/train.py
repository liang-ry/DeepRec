...
class Fat_deepffm():
    def __init__(self,
        ...
        self._ffm_column = ffm_column

        if not wide_column or not ffm_column or not deep_column:
            raise ValueError(
    ...
    def _cen(self, emb_inputs):
        reshape_emb = tf.reshape(emb_inputs,(-1,len(self._ffm_column),len(self._ffm_column),16))
        pooled_inputs = tf.reduce_mean(reshape_emb,3)
        attn_w = self._dnn(tf.reshape(pooled_inputs,(-1,len(self._ffm_column)*len(self._ffm_column))),
                            dnn_hidden_units=[len(self._ffm_column)*len(self._ffm_column)/2,len(self._ffm_column)*len(self._ffm_column)],
                            layer_name='cen') 
        attn_w = tf.expand_dims(attn_w,2)
        outputs = tf.einsum('ijk,ijh->ijk', tf.reshape(emb_inputs,(-1,len(self._ffm_column)*len(self._ffm_column),16)), attn_w)
        return outputs

    def _ffm(self,inputs):
        outputs = []
        field_emb_inputs = tf.reshape(inputs,(-1,len(self._ffm_column),len(self._ffm_column),16))
        for i in range(len(self._ffm_column) - 1):
            for j in range(i + 1, len(self._ffm_column)):
                fij = field_emb_inputs[:, i, j]
                fji = field_emb_inputs[:, j, i]
                output = tf.einsum('ij,ij->ij', fij, fji)
    ...
    def _create_model(self):
        ...
        with tf.variable_scope('input_layer',
            ...
            if self._adaptive_emb and not self.tf:
                ...
                self._feature, self._wide_column, cols_to_output_tensors=ffm_cols,adaptive_mask_tensors=adaptive_mask_tensors)
            else:
                dnn_input = tf.feature_column.input_layer(self._feature,
                                                          self._deep_column)
                wide_input = tf.feature_column.input_layer(
                self._feature, self._wide_column, cols_to_output_tensors=ffm_cols)
            ffm_input = tf.stack([ffm_cols[cols] for cols in self._ffm_column], 1)

        if self.bf16:
            wide_input = tf.cast(wide_input, dtype=tf.bfloat16)
            ffm_input = tf.cast(ffm_input, dtype=tf.bfloat16)
        ...
        # cen part
        with tf.variable_scope('cen', reuse=tf.AUTO_REUSE) as cen:
            cen_output = self._cen(ffm_input)

        # ffm part
        with tf.variable_scope('ffm', reuse=tf.AUTO_REUSE) as ffm:
            ffm_output = self._ffm(cen_output)

        # Final dnn layer
        all_input = tf.concat([dnn_output, linear_output, tf.reduce_sum(ffm_output,2),tf.reduce_sum(cen_output,2)], 1)
...
def build_feature_columns():
        ...
        if column_name in CATEGORICAL_COLUMNS:
            ...
            if args.tf or not args.emb_fusion:
                embedding_column = tf.feature_column.embedding_column(
                    categorical_column,
                    dimension=16*len(CATEGORICAL_COLUMNS),
                    combiner='mean')
            ...
            ffm_column.append(embedding_column)
        else:
    ...
    return wide_column, ffm_column, deep_column
...
def main(tf_config=None, server=None):
    ...
    model_dir = os.path.join(args.output_dir,
                             'model_Fat-deepffm_' + str(int(time.time())))
    ...
    wide_column, ffm_column, deep_column = build_feature_columns()
    ...
    # create model
    model = Fat_deepffm(wide_column=wide_column,
                   ffm_column=ffm_column,
